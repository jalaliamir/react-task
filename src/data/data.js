export const projects = [
  {
    _id: 1,
    title: 'طراحی اپلیکیشن سفر آنلاین',
    description: 'طراحی رابط کاربری و تجربه کاربری اپلیکیشن سفر آنلاین',
    status: 'CLOSED',
    category: {
      id: 1,
      title: 'طراحی UI/UX',
      englishTitle: 'design-ui/ux',
    },
    budget: 10000,
    tags: ['Ui/UX', 'Figma'],
    deadline: '2023-12-23T12:55:48.740Z',
    createdAt: '2023-10-23T18:18:55.636Z',
    updatedAt: '2024-06-02T13:37:48.468Z',
  },
  {
    _id: 2,
    title: 'توسعه سایت فروشگاهی',
    description: 'یک سایت فروشگاهی کامل با پنل ادمین 1',
    status: 'OPEN',
    category: {
      id: 2,
      title: 'برنامه نویسی وب',
      englishTitle: 'web development',
    },
    budget: 50000,
    tags: ['React', 'Nodejs', 'online shop'],
    deadline: '2023-12-23T12:55:48.740Z',
    createdAt: '2023-10-23T18:18:55.636Z',
    updatedAt: '2024-06-02T13:37:48.468Z',
  },
  {
    _id: 3,
    title: 'توسعه سایت بلاگ',
    description: 'یک سایت فروشگاهی کامل با پنل ادمین',
    status: 'CLOSED',
    category: {
      id: 3,
      title: 'برنامه نویسی وب',
      englishTitle: 'web development',
    },
    budget: 20000,
    tags: ['React', 'Nodejs', 'online shop'],
    deadline: '2023-11-23T12:55:48.740Z',
    createdAt: '2023-09-23T18:18:55.636Z',
    updatedAt: '2024-07-02T13:37:48.468Z',
  },
  {
    _id: 4,
    title: 'طراحی برنامه موبایل دیجی پی',
    description: 'طراحی رابط کاربری و تجربه کاربری اپلیکیشن موبایل دیجی پی',
    status: 'OPEN',
    category: {
      id: 4,
      title: 'طراحی UI/UX موبایل',
      englishTitle: 'design-ui/ux-mobile',
    },
    budget: 70000,
    tags: ['Ui/UX', 'Figma'],
    deadline: '2023-10-23T12:55:48.740Z',
    createdAt: '2023-08-23T18:18:55.636Z',
    updatedAt: '2024-09-02T13:37:48.468Z',
  },
  //  خودتون میتونید دیتاهای دیگه رو اضافه کنید.
];
